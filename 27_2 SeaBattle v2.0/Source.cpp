...
void GameHum(int arrPC[][size], int arrHum[][size], int arrMask[][size]);//  GamePlay Human
void destruction(int arrMask[][size], int arrPC[][size], int y, int x); // Ship Destruction by Human
void GamePC(int arrPC[][size], int arrHum[][size], int arrMask[][size]);//  GamePlay PC
int DecksCheck(int arrHum[][size]); // Check Decks presence at battlefield
void destructionByPC(int arrHum[][size], int y, int x);// Ship Destruction by PC
...
//  GamePlay PC
void GamePC(int arrPC[][size], int arrHum[][size], int arrMask[][size]){
	...
	while (totalDecksHuman != 0 || totalDecksPC != 0){
		...
		if (countBrokenDekcs == 1){
			...
			while (1){
				...
				if (arrHum[y][x] != 6 && arrHum[y][x] != 10 && arrHum[y][x] < 1000 && x>=0 && y>=0 && x<size && y<size)
					break;
		...
		if (countBrokenDekcs>1){
			...
			if (!horizont){
				if (arrHum[yBrockenDeck + 1][xBrokenDeck] != 6 && arrHum[yBrockenDeck + 1][xBrokenDeck] != 10 
					&& arrHum[yBrockenDeck + 1][xBrokenDeck] < 1000 && yBrockenDeck - 1 >= 0 && yBrockenDeck + 1 <size){
				...
				if (arrHum[yBrockenDeck][xBrokenDeck + 1] != 6 && arrHum[yBrockenDeck][xBrokenDeck + 1] != 10 
					&& arrHum[yBrockenDeck][xBrokenDeck + 1] < 1000 && xBrokenDeck - 1 >= 0 && xBrokenDeck+1<size){
...
void destructionByPC(int arrHum[][size], int y, int x){
			...
			if (arrHum[i][j] == 10){
				for (int n = i - 1; n <= i + 1; n++){
					for (int k = j - 1; k <= j + 1; k++){
						if (arrHum[n][k] != 10 && n >= 0 && k >= 0 && n < size && k < size)
							arrHum[n][k] = 6;
					}
				}
			}//end of
...
int DecksCheck(int arrHum[][size]){
	int count = 0;
...
//  GamePlay Human
void GameHum(int arrPC[][size], int arrHum[][size], int arrMask[][size]){
	...
	while (inpt != 27 || totalDecksHuman != 0 || totalDecksPC != 0){
		...
		// Movement Direction
		if (inpt == 77 && x<size-1)// move right	
			x++;
		if (inpt == 75 && x>0)// move left
			x--;
		if (inpt == 80 && y<size-1)// move down
			y++;
		...
		for (int i = 0; i < size; i++){
			for (int j = 0; j < size; j++){
				if (arrMask[i][j] == 10){
					for (int n = i - 1; n <= i + 1; n++){
						for (int k = j - 1; k <= j + 1; k++){
							if (arrMask[n][k] != 10 && n >= 0 && k >= 0 && n < size && k < size)
								arrMask[n][k] = 6;
						}
					}
				}//end of
...
// Ship Destruction
void destruction(int arrMask[][size], int arrPC[][size], int y, int x){
			...
			for (int i = 0; i < size; i++){
				for (int j = 0; j < size; j++){
					if (arrPC[i][j] >=100){
						for (int n = i - 1; n <= i + 1; n++){
							for (int k = j - 1; k <= j + 1; k++){
								if (arrPC[n][k] < 100 && n >= 0 && k >= 0 && n < size && k < size)
									arrPC[n][k] = -1;
							}
						}
					}//end of
		...
		for (int i = 0; i < size; i++){
			for (int j = 0; j < size; j++){
				if (arrHum[i][j] >= 100){
					for (int n = i - 1; n <= i + 1; n++){
						for (int k = j - 1; k <= j + 1; k++){
							if (arrHum[n][k] < 100 && n >= 0 && k >= 0 && n < size && k < size)
								arrHum[n][k] = -1;
						}
					}
				}//end of
